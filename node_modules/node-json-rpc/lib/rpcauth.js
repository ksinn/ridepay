...
var Auth = function (options) {
    ...
    var validate = function (user, col) {
        var out = null;


        if (ext.users)
            out = ext.users(user, col);
        else {
            for (var i = 0; i < users.length; i++) {
                if (users[i].login === user[0]) {
                    if (ext.hash) {
    ...
    var checkUser = function (req, col) {
        var header = req.headers['authorization'];

        if (header) {
            validate(
                    new Buffer(header.split(" ")[1], 'base64').toString('utf8').split(':'),
                    col);
        } else {
            col(null);
        }
    ...
    var requestUser = function (res) {
        ...
        //res.writeHead(401);
        res.end('{"jsonrpc":"2.0","error":{"code":-32504, "message" : "Authentication failed"}}');
        //res.end();
    };
    
    var requestUserOld = function (res) {
        res.setHeader("WWW-Authenticate", 'Basic realm="' + message + '"');
        res.writeHead(401);
        res.end(body);
    };
    ...
    this.check = function (req, res, callback) {
        ...
        checkUser(req, function (user) {
            if (user) {
                if (ext.log) {
                    ext.log(req, res, user, callback);
                } else {
                    callback(req, res, user);
                }
            } else {
                if (ext.log) {
                    ext.log(req, res, null, null);
                } else {
                    requestUser(res);
                }
            }
     ...
     this.checkOld = function (req, res, callback) {
     var user = null;
